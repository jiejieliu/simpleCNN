...
padding的另一个可选参数为“VALID”，和“SAME”不同的是：
不用0来填充边界，这时得到的图像的规格就会小于原图像。
新图像尺寸大小 = 原数据尺寸大小 - 卷积核尺寸大小 + 1
（一般我们选用的padding都为“SAME”"""""
...
W_conv1 = weight_variable([5, 5, 1, 32])  # 这里的卷积核大小是5*5的，输入的通道数是1，输出的通道数是32
b_conv1 = bias_variable([32])
h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1)  # output size 28*28*32
h_pool1 = max_pool_2x2(h_conv1)  # 再经过第一次池化（池化步长是2），图片尺寸为14*14*32, output size 14*14*32#卷积层2网络结构定义
